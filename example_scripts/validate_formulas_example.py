...
pb_lin = np.outer(n[0] ** 2, ten_lin[:, 0, 0]) + np.outer(n[1] ** 2, ten_lin[:, 1, 1]) + np.outer(n[2] ** 2,
                                                                                                  ten_lin[:, 2,
                                                                                                  2]) + 2 * np.outer(
    n[0] * n[1], ten_lin[:, 0, 1]) + 2 * np.outer(n[0] * n[2], ten_lin[:, 0, 2]) + 2 * np.outer(n[1] * n[2],
                                                                                                ten_lin[:, 1, 2])
pb_pla = np.outer(n[0] ** 2, ten_pla[:, 0, 0]) + np.outer(n[1] ** 2, ten_pla[:, 1, 1]) + np.outer(n[2] ** 2,
                                                                                                  ten_pla[:, 2,
                                                                                                  2]) + 2 * np.outer(
    n[0] * n[1], ten_pla[:, 0, 1]) + 2 * np.outer(n[0] * n[2], ten_pla[:, 0, 2]) + 2 * np.outer(n[1] * n[2],
                                                                                                ten_pla[:, 1, 2])
